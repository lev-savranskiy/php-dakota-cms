 ...
 *
 */
class Embed_Video extends Model {
     ...
     */
    public function getCode($addHtmlBefore = false, $addHtmlAfter = false, $width = false, $height = false) {
        $objectAttributes = '';
        $objectParams = '';
        $embedAttributes = '';

        if ($width) $this->setEmbedAttr('width', $width)  ;
        if ($height) $this->setEmbedAttr('height', $height)  ;
            ...
            foreach ($this->objectAttr AS $k => $v) {
                     ...
                     if($k == 'width'){
                               ...
                               $objectAttributes .= ' ' . $k . '="' . $width . '"';
                     }
                    elseif($k == 'height'){

                               $objectAttributes .= ' ' . $k . '="' . $height . '"';
                     }
                     else{
            ...
            return sprintf("%s%s\n <object %s>\n %s \n<embed %s />\n</object>%s", $beforeHtml, $this->copyright, $objectAttributes, $objectParams, $embedAttributes, $afterHtml);
        }
         ...
         */
        public
        function parseText($text, $width , $height ,  $addHtmlBefore = false, $addHtmlAfter = false) {

            if (preg_match_all('/(https?[\S]+)/i', $text, $result)) {
                foreach ($result[1] AS $link) {
                    if ($this->parseUrl($link)) {
                        $linkReplacement = $this->getCode($addHtmlBefore , $addHtmlAfter , $width, $height);
         ...
         */
        public
        function parseHtml($html, $keepLink = false, $addHtmlBefore = false, $addHtmlAfter = false) {
         ...
         */
        public
        function setBeforeHtml($beforeHtml) {
         ...
         */
        public
        function setAfterHtml($afterHtml) {
